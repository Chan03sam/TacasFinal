...
class Login extends DBConnection {
	...
	function outsiders_login(){
		extract($_POST);
		$qry = $this->conn->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as fullname from outsiders_list where email = '$email' and `password` = md5('$password') ");
		if($this->conn->error){
			$resp['status'] = 'failed';
			$resp['msg'] = "An error occurred while fetching data. Error:". $this->conn->error;
		}else{
	...
	public function outsiders_logout(){
		if($this->settings->sess_des()){
			redirect('./');
		}
	}
...
switch ($action) {
	...
	case 'student_logout':
		...
		break;
		case 'outsiders_login':
			echo $auth->outsiders_login();
			break;
		case 'outsiders_logout':
			echo $auth->outsiders_logout();
			break;
	default:
		echo $auth->index();
		break;
